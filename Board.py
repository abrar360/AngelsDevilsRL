...
class Board:
    def __init__(self, margin, sides, ql, angel_trained=False, devil_trained=False):
        self.margin = margin
        self.sides = sides
    ...
    def get_devil_blocks(self):
        return self.devil.get_blocks()[-1]
    ...
    def check_winner(self, current_player):
        if current_player == "angel":
            if self.angel.get_position() in self.devil.get_blocks():
            ...
            elif self.angel.get_position() == 101:
                self.winner = "devil"
                self.reason = "angel trapped"
                ...
                self.winner = "angel"
                self.reason = "angel escaped"
            elif self.ql.get_layer(self.angel.get_position()) == 4:
                self.winner = "angel"
                self.reason = "angel escaped"
    ...
    def angels_turn(self):
        self.angel.angel_move(self.representation(), self.ql)
